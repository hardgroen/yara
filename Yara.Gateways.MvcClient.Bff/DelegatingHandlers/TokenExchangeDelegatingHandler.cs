...
namespace Yara.Gateways.MvcClient.Bff.DelegatingHandlers
{
    public class TokenExchangeDelegatingHandler : DelegatingHandler
    {
        ...
        public async Task<string> GetAccessToken(string incomingToken)
        {
            ...
            var item = await _clientAccessTokenCache
                .GetAsync("mvcclientbfftodownstreamtokenexchangeclient_postings", catp);
            if (item != null)
            {
            ...
            await _clientAccessTokenCache.SetAsync(
                "mvcclientbfftodownstreamtokenexchangeclient_postings",
                accessToken,
                expiresIn,
                catp);
